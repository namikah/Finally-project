import React from "react";
import "./socials.scss";

function Socials() {
  return (
    <div
      id="socials-icons"
      className="col-lg-4 col-md-12 d-flex justify-content-center align-items-center gap-3 pb-2"
    >
      <a
        target="_blank"
        rel="noreferrer"
        href="https://fb.com"
      >
        <img
          src="https://www.cinemaplus.az/site/templates/images/fb.png"
          alt="social-icon"
        ></img>
      </a>
      <a
        target="_blank"
        rel="noreferrer"
        href="https://twitter.com"
      >
        <img
          src="data:image/svg+xml;base64,"
          alt="social-icon"
        ></img>
      </a>
      <a
        target="_blank"
        rel="noreferrer"
        href="https://youtube.com"
      >
        <img
          src="https://www.cinemaplus.az/site/templates/images/youtube.png"
          alt="social-icon"
        ></img>
      </a>
      <a
        target="_blank"
        rel="noreferrer"
        href="https://instagram.com"
      >
        <img
          src="https://www.cinemaplus.az/site/templates/images/instagram.png"
          alt="social-icon"
        ></img>
      </a>
    </div>
  );
}

export default Socials;
