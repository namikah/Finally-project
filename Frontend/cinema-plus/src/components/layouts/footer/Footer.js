import React from "react";
import { Link } from "react-router-dom";
import "./footer.scss";

function Footer() {
  return (
   <footer>
     <div className="top-footer row">
       <div className="left-side col-md-4 d-flex justify-content-center align-items-center gap-5">
         <ul>
           <li>
             <Link>Haqqımızda</Link>
           </li>
           <li>
             <Link>Xidmətlər</Link>
           </li>
           <li>
             <Link>Haqqımızda</Link>
           </li>
         </ul>
         <ul>
           <li>
             <Link>FAQ</Link>
           </li>
           <li>
             <Link>Vakansiyalar</Link>
           </li>
           <li>
             <Link>Əlaqə</Link>
           </li>
         </ul>
       </div>
       <div className="middle-side col-md-4 d-flex justify-content-center align-items-center gap-3">
         <Link>
         <img src="https://www.cinemaplus.az/site/templates/images/fb.png" alt="social-icon"></img>
         </Link>
         <Link>
         <img src="data:image/svg+xml;base64," alt="social-icon"></img>
         </Link>
         <Link>
         <img src="https://www.cinemaplus.az/site/templates/images/youtube.png" alt="social-icon"></img>
         </Link>
         <Link>
         <img src="https://www.cinemaplus.az/site/templates/images/instagram.png" alt="social-icon"></img>
         </Link>
       </div>
       <div className="col-md-4"></div>
     </div>
     <div className="bottom-footer"></div>
   </footer>
  );
}

export default Footer;
